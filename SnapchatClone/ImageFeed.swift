            ...
            if let posts = snapshot.value as? [String:AnyObject] {
                user.getReadPostIDs(completion: { (ids) in
                    for postKey in posts.keys {
                        ...
                        let p = Post(id: postKey
                            , username: posts[postKey]![firUsernameNode] as! String
                            , postImagePath: posts[postKey]![firImagePathNode] as! String
                            , thread: posts[postKey]![firThreadNode] as! String
                            , dateString: posts[postKey]![firDateNode] as! String, read: read)
                        postArray.append(p)
                    }
